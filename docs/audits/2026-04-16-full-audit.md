...
| 🔴 | Auth | AC-6: Sign-out is not reachable from any UI — `clearAuth()` exists in the store but no screen, button, or menu item calls it; the user has no way to sign out | `rn-app/app/(app)/index.tsx` | Add a sign-out button to the home screen (or a profile/settings screen) that calls `useAuthStore().clearAuth()` | S |
