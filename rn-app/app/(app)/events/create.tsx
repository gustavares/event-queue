...
export default function CreateEventScreen() {
    ...
    const [tiers, setTiers] = React.useState<Tier[]>([{ name: '', price: '' }]);
    ...
    const handleSubmit = async () => {
        ...
        if (result.error || !result.data?.createEvent) {
            setSubmitting(false);
            return;
        }
        ...
        if (doorSalesEnabled) {
            const validTiers = tiers.filter((t) => t.name.trim() && t.price.trim());
        ...
        router.replace(`/(app)/events/${eventId}` as any);
    };
